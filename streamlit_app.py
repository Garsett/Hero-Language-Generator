...
try:
    ...
    # Dit wordt getoond als de sleutel niet is ingesteld in Hugging Face Secrets
    st.error("Google API-sleutel niet gevonden. Zorg ervoor dat je de GOOGLE_API_KEY hebt ingesteld in de 'Settings' > 'Secrets' van je Hugging Face Space.")
    st.stop()
...
def generate_ai_lesson(niveau, les, hero):
    ...
    # Gebruik het nieuwste, stabiele model dat werkt met de moderne bibliotheek
    ...
    **Instructies voor de les:**
    ...
    with st.spinner(f"✨ Magie in de maak... Ik genereer een les over '{les}' voor {hero['name']}..."):
        try:
            ...
            # Toon een gedetailleerde foutmelding direct in de app
